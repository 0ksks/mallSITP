...
def combine(path:str)->pd.DataFrame:
    ...
    if "会员主档资料" in path:
        ...
        types = "category datetime64[ns] category category category category float16 category category datetime64[ns] float16 category category category category category category category category int16 category category"
    elif "会员消费积分报表" in path:
        types = "category category datetime64[ns] float16 float16 category category"
    else:
        dfs["年龄"].fillna(0, inplace=True)
        dfs["消费笔数 总消费额 积分兑换次数 积分余额".split()].fillna(0, inplace=True)
        types = "category datetime64[ns] datetime64[ns] category category category category category float16 category category category category float16 float16 float16 float16"
